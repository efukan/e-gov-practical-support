...
(function(ext) {
   ...
   */
  ext.definitionTooltipEl = null;

  /**
   * 定義語ポップアップのイベントが登録済みかどうかのフラグ
   * @type {boolean}
   */
  ext.definitionTooltipEventsAdded = false;
  ...
  ext.extractDefinitionsAsync = async function() {
    ...
    const addDefinition = (word, data) => {
      ...
      const existing = ext.definitionMap.get(word);
      if (existing) {
        if (!existing.elements) {
          existing.elements = [existing.element];
        }
        if (data.element && !existing.elements.includes(data.element)) {
          existing.elements.push(data.element);
        }
      } else {
        data.elements = data.element ? [data.element] : [];
        ext.definitionMap.set(word, data);
      }
    };

    const container = ext.deepQuerySelectorAll(document.body, '.LawBody')[0] || 
                      ...
                      document.body;
    if (!container) {
      console.log("egov-ext: No container found for definition extraction.");
      return;
    }
    console.log("egov-ext: Selected container:", container.tagName, "class:", container.className, "id:", container.id);
    ...
    console.log("egov-ext: Found inline elements for Pattern 1:", inlineElements.length);
    ...
    console.log("egov-ext: Found sentence elements for Pattern 2:", sentenceElements.length);
    ...
    console.log("egov-ext: Found item elements for Pattern 3:", itemElements.length);
    ...
    console.log("egov-ext: Found item sentences for Pattern 4:", itemSentences.length);
    ...
    console.log("egov-ext: Extraction complete (async). Total terms extracted:", ext.definitionMap.size, Array.from(ext.definitionMap.keys()));
  };
  ...
  ext.enableDefinitionHighlighting = function(targetContainer = null) {
    ...
    if (ext.definitionMap.size === 0) {
      console.log("egov-ext: No defined terms extracted. Skipping highlighting.");
      return;
    }
    ...
    const container = targetContainer || 
                      ext.deepQuerySelectorAll(document.body, '.LawBody')[0] || 
                      ext.deepQuerySelectorAll(document.body, '.main-content')[0] || 
                      ext.deepQuerySelectorAll(document.body, '.provisiontext')[0] || 
                      ext.deepQuerySelectorAll(document.body, 'article.law')[0] || 
                      document.body;
    ...
    const blocks = leafBlocks.filter(block => !ext.deepClosest(block, excludedSelectors));
    ...
    const processHighlight = (block) => {
      ...
      ext.definitionMap.forEach((def, term) => {
        if (def.pattern === 1 && (def.element || def.elements)) {
          const elements = def.elements && def.elements.length > 0 ? def.elements : [def.element];
          const isAtOrAfter = elements.some(el => {
            return el === block || (el.compareDocumentPosition(block) & Node.DOCUMENT_POSITION_FOLLOWING);
          });
          if (isAtOrAfter) {
      ...
      nodes.forEach(node => {
        ...
        while ((match = regex.exec(text)) !== null) {
          ...
          const isKanji = (char) => char && /\p{Script=Han}/u.test(char);
          const isKatakana = (char) => char && /[\p{Script=Katakana}ー]/u.test(char);

          const allowedPrefixRegex = /(当該|同|本|各|前|新|旧|元|全|現|同一|同等|同種|共同|連帯|根|第三|代表|社外|他)$/;
          const allowedSuffixRegex = /^(等|及び|若しくは|又は|並びに|対策|対応|活用|管理|整備|設計|設置|関係|行政|支援|推進)/;
          ...
          // 許可された接尾辞が「名詞系」であるか判定する正規表現
          const nounSuffixRegex = /^(対策|対応|活用|管理|整備|設計|設置|関係|行政|支援|推進)/;

          // 許可された接尾辞（名詞系）の後ろに漢字がさらに2文字以上続くかチェック
          let isFollowedByLongKanji = false;
          if (isKanji(nextChar)) {
            const suffixMatch = textAfter.match(allowedSuffixRegex);
            if (suffixMatch) {
              const matchedSuffix = suffixMatch[0];
              // 接尾辞が名詞系の場合のみ、その後の漢字連続数をカウント
              if (nounSuffixRegex.test(matchedSuffix)) {
          ...
          const isFalsePositive = 
            ...
            (isKanji(prevChar) && !allowedPrefixRegex.test(textBefore)) ||
            (isKanji(nextChar) && !allowedSuffixRegex.test(textAfter));
    ...
    if (targetContainer) {
      ...
      ext.runTaskInChunks('definitionHighlight', blocks, processHighlight, 100, (state) => {
        ...
        console.log("egov-ext: Highlighting complete. Highlighted occurrences (async):", highlightedCount);
        setupDefinitionTooltipEvents();
      });
    }
  };
  ...
  /**
   * 定義ホバーのイベント登録及び表示位置制御
   */
  function setupDefinitionTooltipEvents() {
    if (!ext.settings.global || !ext.settings.definition) return;

    if (!ext.definitionTooltipEl || !document.body.contains(ext.definitionTooltipEl)) {
      ext.definitionTooltipEl = document.createElement('div');
      ext.definitionTooltipEl.className = 'egov-ext-definition-tooltip';
      document.body.appendChild(ext.definitionTooltipEl);
    }

    if (ext.definitionTooltipEventsAdded) return;

    document.body.addEventListener('mouseover', (e) => {
      if (!ext.settings.global || !ext.settings.definition) return;

      const target = ext.getComposedTarget(e, '.egov-definition-word');
      if (target) {
        const word = target.dataset.word;
        if (ext.definitionTooltipEl.dataset.activeWord === word && ext.definitionTooltipEl.classList.contains('visible')) {
          return;
        }
        ext.definitionTooltipEl.dataset.activeWord = word;

        const def = ext.definitionMap.get(word);
        if (def && def.element) {
          ext.definitionTooltipEl.textContent = '';

          const header = document.createElement('div');
          header.className = 'egov-ext-definition-tooltip-header';
          header.textContent = def.source || '定義語';
          ext.definitionTooltipEl.appendChild(header);

          const body = document.createElement('div');
          body.className = 'egov-ext-definition-tooltip-body';
          ...
          ext.definitionTooltipEl.appendChild(body);

          const rect = target.getBoundingClientRect();

          ext.definitionTooltipEl.style.left = '0px';
          ext.definitionTooltipEl.style.top = `${rect.bottom + window.scrollY + 5}px`;
          ext.definitionTooltipEl.classList.add('visible');

          const tooltipWidth = ext.definitionTooltipEl.offsetWidth;

          if (rect.left > window.innerWidth / 2) {
            let leftPos = rect.right - tooltipWidth;
            if (leftPos < 10) leftPos = 10;
            ext.definitionTooltipEl.style.left = `${leftPos + window.scrollX}px`;
          } else {
            ext.definitionTooltipEl.style.left = `${rect.left + window.scrollX}px`;
          }
        }
      }
    });
    ...
    document.body.addEventListener('mouseout', (e) => {
      if (!ext.definitionTooltipEl) return;
      
      const related = e.relatedTarget;
      if (!related) {
        ext.definitionTooltipEl.classList.remove('visible');
        ext.definitionTooltipEl.dataset.activeWord = '';
        return;
      }
      
      const toWord = related.closest ? related.closest('.egov-definition-word') : null;
      const toTooltip = (related === ext.definitionTooltipEl) || (ext.definitionTooltipEl.contains && ext.definitionTooltipEl.contains(related));
    ...
    ext.definitionTooltipEventsAdded = true;
  }
