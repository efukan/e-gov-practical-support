...
3. **参照条文ポップアップ**
   ...
   * 長い条文もポップアップ内でスクロールして続きまで読めます。読書中のマウス通過による誤表示を防ぐため、約0.3秒の快適な表示遅延を設けています。
4. **定義語ホバー辞書**
...
9. **被引用（引用元）表示**
   ...
   * ボタンにマウスホバーすることで、その条文を引用している他法令（施行令・施行規則など）の一覧がポップアップで瞬時に表示されます。
   * 一覧内の条文リンクにマウスを乗せると、該当条文の本文が横にフライアウト表示され、引用箇所が自動ハイライトされます（HTML形式の法令も公式XML APIから自動補完）。ポップアップ内にも「横書き表記変換」が適用されます。
